...
osc.on('/select', async message => {
  ...
  await osc.context.client.select(facts).doAll(assertions => {
    send('/assertions', JSON.stringify(assertions))
  })
})
...
osc.on('open', () => {
  ...
  osc.on('updateSubscription', subscriptions => {
    queries =
      typeof subscriptions === 'string' ? [subscriptions] : subscriptions
  })
  ...
  const loop = async () => {
    if (queries && queries.length > 0) {
      ...
      if (await facts.isNotEmpty()) {
        await facts.doAll(assertions => {
          send('/subscriptionFacts', { queries, assertions })
        })
      }
    }

    timeout = setTimeout(loop, 4)
  }
